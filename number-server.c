...
void handle_404(int client_sock, char *path)  {
    ...
    write(client_sock, HTTP_404_NOT_FOUND, strlen(HTTP_404_NOT_FOUND)); //added
    write(client_sock, response_buff, strlen(response_buff));
}

void handle_root(int client_sock){
    char message[BUFFER_SIZE];
    snprintf(message, BUFFER_SIZE, "Surrent number: %d\n", num);
...
void handle_response(char *request, int client_sock) {
    ...
    /*
        ...
        "/increment" - adds 1 to number and shows "Incremented to: ____"
    */

    if(strcmp(path,"/") == 0){
        printf("Here we will handle the root path / \n");
